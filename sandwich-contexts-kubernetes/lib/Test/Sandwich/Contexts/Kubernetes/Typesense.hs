...
withTypesense' kcc _kubectlBinary namespace options@(TypesenseOptions {..}) action = do
  ...
  createProcessWithFileLogging ((proc helmBinary [
    "repo", "add", "springboard", "https://helm-charts.springboardvr.com"
    ]) { env = Just env })
    >>= waitForProcess >>= (`shouldBe` ExitSuccess)

  createProcessWithFileLogging ((proc helmBinary ["repo", "update"]) { env = Just env })
    >>= waitForProcess >>= (`shouldBe` ExitSuccess)
  ...
  createProcessWithFileLogging ((proc helmBinary helmArgs) { env = Just env })
    >>= waitForProcess >>= (`shouldBe` ExitSuccess)
